...
void GameFixes::TryLoadAutoSave() {
	GameUtil::LoadFromFile("chaos_mod\\chaos_autosave.b");
}

void __fastcall GameFixes::HookedProcessMenuOptions(CMenuManager* thisManager, void* edx, eMenuPage page) {
...
bool GameFixes::HookedGetIsSlowMotionActive() {
	return true;
}

int __fastcall GameFixes::HookedRadioSetVolume(uint8_t* thisAudioHardware, void* edx, int a2, int a3, float volume, int a5) {
...
__int16 __fastcall GameFixes::HookedCRunningScriptCollectParameters(CRunningScript* thisScript, void* edx, unsigned __int16 num) {
...
char GameFixes::HookedCheatCancelEachOther(int id) {
	return 0;
}
