...
PinkCarsCheat::PinkCarsCheat()
	: EffectBase("cheat_pink_cars")
{
	AddType("vehicle_color");
}

void PinkCarsCheat::Enable() {
	EffectBase::Enable();
...
void PinkCarsCheat::SetupRenderEvent(CVehicle* vehicle) {
	ModifyCarPaint(vehicle);
}

void PinkCarsCheat::ResetAfterRenderEvent(CVehicle* vehicle) {
...
void PinkCarsCheat::ModifyCarPaint(CVehicle* vehicle) {
...
RpMaterial* PinkCarsCheat::MaterialCallback(RpMaterial* material, void* data) {
	if (!data) {
		return material;
	}
...
RpAtomic* PinkCarsCheat::AtomicCallback(RpAtomic* atomic, void* data) {
	if (atomic->geometry) {
