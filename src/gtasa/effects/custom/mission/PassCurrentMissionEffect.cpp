...
void PassCurrentMissionEffect::InitializeHooks() {
...
void PassCurrentMissionEffect::Enable() {
	EffectBase::Enable();
...
void PassCurrentMissionEffect::Disable() {
	isEnabled = false;

	EffectBase::Disable();
}

void PassCurrentMissionEffect::HandleTick() {
	...
	for (auto i = CTheScripts::pActiveScripts; i; i = i->m_pNext) {
		if (i->m_bIsMission && i->m_bIsActive && !i->m_bIsExternal) {
			...
			if (offsetMap.contains(missionName)) {
				...
				if (missionName == "FINALEC") { // End Of The Line (Part 3)
...
void PassCurrentMissionEffect::ClearPlayerStatus() {
...
char __stdcall PassCurrentMissionEffect::HookedDebugGetKeyDown(int keyCode, int a2, int a3) {
