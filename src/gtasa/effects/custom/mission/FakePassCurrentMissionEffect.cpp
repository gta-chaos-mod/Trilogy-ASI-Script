...
void
FakePassCurrentMissionEffect::HandleTick ()
{
    ...
    if (!handledMission)
    {
        ...
        if (wasOnMission)
        {
            EffectDatabase::QueueEffect (
                new AutosaveEffect (GameUtil::GetRealMissionsPassed ()), true);
        }
    }
    else
    {
...
void
FakePassCurrentMissionEffect::ClearPlayerStatus ()
{
