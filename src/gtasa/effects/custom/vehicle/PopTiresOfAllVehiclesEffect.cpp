...
void PopTiresOfAllVehiclesEffect::HandleTick() {
	...
	for (CVehicle* vehicle : CPools::ms_pVehiclePool) {
		...
		if (CModelInfo::IsCarModel(vehicle->m_nModelIndex)) {
			...
			if (!car->m_damageManager.GetWheelStatus(eWheels::WHEEL_FRONT_LEFT)) {
			...
			if (!car->m_damageManager.GetWheelStatus(eWheels::WHEEL_REAR_RIGHT)) {
