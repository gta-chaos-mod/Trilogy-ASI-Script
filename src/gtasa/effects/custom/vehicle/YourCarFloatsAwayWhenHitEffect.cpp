...
void YourCarFloatsAwayWhenHitEffect::Enable() {
	EffectBase::Enable();

	applyCollisionEvent += ApplyCollision;
}

void YourCarFloatsAwayWhenHitEffect::Disable() {
	applyCollisionEvent -= ApplyCollision;
...
void YourCarFloatsAwayWhenHitEffect::HandleTick() {
...
void YourCarFloatsAwayWhenHitEffect::ApplyCollision(CVehicle* thisVehicle, CVehicle* a2, int a3, float* a4, int a5) {
	CVehicle* playerVehicle = FindPlayerVehicle(-1, false);
	if (!playerVehicle) return;

	if (playerVehicle == thisVehicle && a2->m_nType == eEntityType::ENTITY_TYPE_VEHICLE
		|| playerVehicle == a2 && thisVehicle->m_nType == eEntityType::ENTITY_TYPE_VEHICLE) {
