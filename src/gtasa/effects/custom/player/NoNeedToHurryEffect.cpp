...
void
NoNeedToHurryEffect::Disable ()
{
    for (CVehicle *vehicle : CPools::ms_pVehiclePool)
    {
        Command<
            eScriptCommands::COMMAND_SET_VEHICLE_AIR_RESISTANCE_MULTIPLIER> (
            vehicle, 0.0f);
    }
...
void
NoNeedToHurryEffect::HandleTick ()
{
    ...
    if (vehicle->m_pDriver != FindPlayerPed()) {
            Command<eScriptCommands::COMMAND_SET_CAR_MISSION>(vehicle,
    eCarMission::MISSION_ATTACKPLAYER);
    }
