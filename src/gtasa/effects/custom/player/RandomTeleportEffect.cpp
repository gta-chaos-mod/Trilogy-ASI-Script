...
RandomTeleportEffect::HandleTick ()
{
    ...
    if (player && Teleportation::CanTeleport())
    {
        ...
        Command<eScriptCommands::COMMAND_REQUEST_COLLISION> (randomPosition.x, randomPosition.y);
        ...
        randomPosition.z = CWorld::FindGroundZForCoord (randomPosition.x,
                                                        randomPosition.y) + 3.0f;

        float pLevel = 0;
        Call<0x6EB690> (randomPosition.x, randomPosition.y, randomPosition.z, &pLevel, false, 1); // CWaterLevel::GetWaterLevel
