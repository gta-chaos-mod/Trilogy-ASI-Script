...
void InvertedControlsEffect::InitializeHooks() {
...
void InvertedControlsEffect::Enable() {
	EffectBase::Enable();
...
void InvertedControlsEffect::Disable() {
	for (int i = 0; i < 59; i++) {
...
void InvertedControlsEffect::HandleTick() {
...
void InvertedControlsEffect::SwapControls(e_ControllerAction target, e_ControllerAction source) {
	invertedActions[(int)target] = origActions[(int)source];
	invertedActions[(int)source] = origActions[(int)target];
}

FILESTREAM InvertedControlsEffect::HookedOpenFile(const char* file, const char* mode) {
	return 0;
}

void __fastcall InvertedControlsEffect::HookedCMenuManagerProcessPCMenuOptions(CMenuManager* thisManager, void* edx, eMenuPage page) {
