...
void HighPitchedAudioEffect::InitializeHooks() {
...
void HighPitchedAudioEffect::Disable() {
	injector::WriteMemory(0x8CBA6C, 1.0f, true);

	EffectBase::Disable();
}

void HighPitchedAudioEffect::HandleTick() {
	EffectBase::HandleTick();

	injector::WriteMemory(0x8CBA6C, audioPitch, true);
}

int __fastcall HighPitchedAudioEffect::HookedSetFrequencyScalingFactor(DWORD* thisAudioHardware, void* edx, int slot, int offset, float factor) {
...
int __fastcall HighPitchedAudioEffect::HookedRadioSetVolume(uint8_t* thisAudioHardware, void* edx, int a2, int a3, float volume, int a5) {
