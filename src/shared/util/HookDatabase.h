...
private:
    struct HookElement
    {
        ...
        HookElement (){};
        HookElement (std::string id, int address,
                     injector::memory_pointer_raw pointer, bool isCall)
        {
            this->id      = std::move (id);
    ...
    static std::unordered_map<int, HookElement> &
    registeredHooks ()
    {
...
public:
    static void HookCall (std::string id, int address, void *func,
                          bool vp = true);

    static void HookJump (std::string id, int address, void *func,
                          bool vp = true);
