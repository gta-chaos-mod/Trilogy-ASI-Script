...
EffectDatabase::Tick ()
{
    ...
    effects.remove_if (
        [] (EffectBase *effect) { return !effect->IsRunning (); });
}

void
EffectDatabase::EmptyQueue ()
{
...
void
EffectDatabase::QueueFunction (_Callable &&__f, _Args &&... __args)
{
...
void
EffectDatabase::QueueEffect (EffectBase *effect, bool executeNow)
{
    if (!effect)
    {
        return;
...
void
EffectDatabase::HandleFunction (std::string state, std::string text)
{
    char c_function[128]    = {};
    int  duration           = 0;
...
std::list<EffectBase *>
EffectDatabase::GetActiveEffects ()
{
    return effects;
}
