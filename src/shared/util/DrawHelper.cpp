...
std::string DrawHelper::GetCooldownString() {
	std::string cooldown;
	if (mode != "") {
...
float DrawHelper::CalculateBarWidth() {
	return (SCREEN_WIDTH / cooldown * remaining);
}

void DrawHelper::UpdateCooldown(int _remaining, int _cooldown, std::string _mode) {
	remaining = _remaining;
	cooldown = _cooldown;
	mode = _mode;
}

void DrawHelper::Draw() {
	DrawVersion();
...
void DrawHelper::DrawVersion() {
	if (GenericUtil::IsMenuActive() && KeyPressed(VK_F7)) {
		gamefont::Print(gamefont::LeftBottom, gamefont::AlignLeft, GenericUtil::GetModVersion(), 20.0f, 60.0f, FONT_DEFAULT, 1.0f, 1.8f, CRGBA(255, 255, 255, 255), 2, CRGBA(0, 0, 0, 255), true);
	}
}

void DrawHelper::DrawTopBar() {
	...
	rect = CRect(SCREEN_COORD_LEFT(4.0f), SCREEN_COORD_TOP(4.0f), SCREEN_WIDTH - SCREEN_COORD(4.0f), SCREEN_COORD_TOP(26.0f));
	CSprite2d::DrawRect(rect, CRGBA(25, 75, 128, 200));

	rect = CRect(SCREEN_COORD_LEFT(4.0f), SCREEN_COORD_TOP(4.0f), barWidth + SCREEN_COORD(4.0f), SCREEN_COORD_TOP(26.0f));
	CSprite2d::DrawRect(rect, CRGBA(50, 150, 255, 200));

	float x = SCREEN_COORD_CENTER_X;
	gamefont::PrintUnscaled(GetCooldownString(), x, SCREEN_MULTIPLIER(2.0f), FONT_DEFAULT, SCREEN_MULTIPLIER(0.8f), SCREEN_MULTIPLIER(0.8f),
		CRGBA(255, 255, 255, 255), gamefont::AlignCenter, 1, CRGBA(0, 0, 0, 255), true, 9999.0F, false);
...
void DrawHelper::DrawRecentEffects() {
	...
	for (EffectBase* effect : EffectDatabase::GetActiveEffects()) {
		...
		if (!effect->IsPlaceholder() && (effect->IsEnabled() || effect->IsDisabledForMissions())) {
			isInset = true;
			break;
		}
	}
	...
	for (EffectBase* effect : EffectDatabase::GetActiveEffects()) {
		...
		bool timedEffect = !effect->IsPlaceholder() && (effect->IsEnabled() || effect->IsDisabledForMissions());

		float position = isInset ? 90.0f : 25.0f;

#ifdef GTASA
		CFont::SetScaleForCurrentlanguage(1.0f, 1.2f);
		float renderWidth = CFont::GetStringWidth((char*)effect->GetDescription().c_str(), true, false) + position;
#elif GTAVC
		CFont::SetScale(1.0f, 1.2f);
		std::string description = effect->GetDescription();
		float renderWidth = CFont::GetStringWidth((wchar_t*)std::wstring(description.begin(), description.end()).c_str(), true) + position;
#elif GTA3
		CFont::SetScale(1.0f, 1.2f);
		float renderWidth = CFont::GetStringWidth((char*)effect->GetDescription().c_str(), true) + position;
#endif
		float offsetWidth = effect->CalculateFadeInOffset(renderWidth + position);

		float x = offsetWidth - renderWidth;
		float y = ((i + 1) * 65.0f) + 240.0f;

		gamefont::Print(gamefont::RightBottom, gamefont::AlignRight, effect->GetDescription(), x, y, FONT_DEFAULT, 1.0f, 1.2f, effect->GetTextColor(), 1, CRGBA(0, 0, 0, 255), true);

		// Draw voter
		if (effect->HasTwitchVoter()) {
			gamefont::Print(gamefont::RightBottom, gamefont::AlignRight, effect->GetTwitchVoter(), x, y - 30.0f, FONT_DEFAULT, 0.8f, 1.0f, effect->GetTextColor(), 1, CRGBA(0, 0, 0, 255), true);
		}
		...
		if (timedEffect) {
			if (Config::GetOrDefault("Drawing.DrawCircles", true)) {
				...
				CVector2D center = CVector2D(SCREEN_COORD_RIGHT(x) + SCREEN_COORD(50.0f), SCREEN_COORD_BOTTOM(y) + SCREEN_COORD(20.0f));
				...
				float angle = (float)effect->GetEffectRemaining() / (float)effect->GetEffectDuration() * 360.0f;

				DrawHelper::DrawCircle(center, SCREEN_MULTIPLIER(24.0f), 360.0f, CRGBA(255, 255, 255, 255));
				...
				DrawHelper::DrawCircle(center, SCREEN_MULTIPLIER(20.0f), 360.0f, backgroundColor);

				DrawHelper::DrawCircle(center, SCREEN_MULTIPLIER(20.0f), angle, effect->GetEffectColor());

				if (effect->GetEffectRemaining() <= 60000) {
					gamefont::Print(gamefont::RightBottom, gamefont::AlignRight, GenericUtil::FormatTime(effect->GetEffectRemaining(), true), x - 57.0f, y - 2.0f, FONT_DEFAULT, 0.6f, 1.0f, CRGBA(255, 255, 255, 200), 1, CRGBA(0, 0, 0, 200), true, 9999.0f, false);
				}
			}
			else {
				gamefont::Print(gamefont::RightBottom, gamefont::AlignRight, GenericUtil::FormatTime(effect->GetEffectRemaining()), x - 60.0f, y - 10.0f, FONT_DEFAULT, 0.6f, 0.8f, effect->GetTextColor(), 1, CRGBA(0, 0, 0, 255), true, 9999.0f, false);
			}
		}
