...
namespace cpptoml
{
    ...
    {
        static inline struct offset_datetime
        from_zoned (const struct tm &t)
        {
            offset_datetime dt;
        ...
        static inline struct offset_datetime
        from_local (const struct tm &t)
        {
            return from_zoned (t);
        }

        static inline struct offset_datetime
        from_utc (const struct tm &t)
        {
            offset_datetime dt;
            dt.year   = t.tm_year + 1900;
            dt.month  = t.tm_mon + 1;
    ...
    inline std::ostream &
    operator<< (std::ostream &os, const local_date &dt)
    {
        fill_guard g{ os };
        os.fill ('0');

        using std::setw;
        os << setw (4) << dt.year << "-" << setw (2) << dt.month << "-"
           << setw (2) << dt.day;
    ...
    inline std::ostream &
    operator<< (std::ostream &os, const local_time &ltime)
    {
        fill_guard g{ os };
        os.fill ('0');
    ...
    inline std::ostream &
    operator<< (std::ostream &os, const zone_offset &zo)
    {
        fill_guard g{ os };
        os.fill ('0');
    ...
    inline std::ostream &
    operator<< (std::ostream &os, const local_datetime &dt)
    {
    ...
    inline std::ostream &
    operator<< (std::ostream &os, const offset_datetime &dt)
    {
    ...
    template <class T>
    struct valid_value
        : is_one_of<T, std::string, int64_t, double, bool, local_date,
    ...
    /// Base type used to store underlying data type explicitly if RTTI is
    /// disabled
    ...
    public:
        ...
        {
            if (is_table_array ())
                return std::static_pointer_cast<table_array> (
                    shared_from_this ());
            return nullptr;
        }
    ...
    template <class T>
    std::shared_ptr<typename value_traits<T>::type>
    make_value (T &&val)
    {
        ...
        return std::make_shared<value_type> (enabler{},
                                             value_traits<T>::construct (
                                                 std::forward<T> (val)));
    }

    template <class T>
    inline std::shared_ptr<value<T>>
    base::as ()
    {
#if defined(CPPTOML_NO_RTTI)
    ...
    // double value
    template <>
    inline std::shared_ptr<value<double>>
    base::as ()
    {
#if defined(CPPTOML_NO_RTTI)
        if (type () == base_type::FLOAT)
            return std::static_pointer_cast<value<double>> (
                shared_from_this ());

        if (type () == base_type::INT)
        {
            auto v = std::static_pointer_cast<value<int64_t>> (
                shared_from_this ());
            return make_value<double> (static_cast<double> (v->get ()));
        }
#else
        if (auto v
            = std::dynamic_pointer_cast<value<double>> (shared_from_this ()))
            return v;

        if (auto v
            = std::dynamic_pointer_cast<value<int64_t>> (shared_from_this ()))
            return make_value<double> (static_cast<double> (v->get ()));
#endif
    ...
    template <class T>
    inline std::shared_ptr<const value<T>>
    base::as () const
    {
#if defined(CPPTOML_NO_RTTI)
        if (type () == base_type_traits<T>::type)
            return std::static_pointer_cast<const value<T>> (
                shared_from_this ());
        else
            return nullptr;
#else
    ...
    // double value
    template <>
    inline std::shared_ptr<const value<double>>
    base::as () const
    {
#if defined(CPPTOML_NO_RTTI)
        if (type () == base_type::FLOAT)
...
#else
        if (auto v = std::dynamic_pointer_cast<const value<double>> (
                shared_from_this ()))
            return v;
    ...
    public:
        ...
        using const_iterator
            = std::vector<std::shared_ptr<base>>::const_iterator;
        ...
        template <class T>
        void
        push_back (const std::shared_ptr<value<T>> &val)
        {
         ...
         */
        void
        push_back (const std::shared_ptr<array> &val)
        {
        ...
        template <class T>
        void
        push_back (T &&val, typename value_traits<T>::type * = 0)
        {
        ...
        template <class T>
        iterator
        insert (iterator position, const std::shared_ptr<value<T>> &value)
        {
            if (values_.empty () || values_[0]->as<T> ())
            {
         ...
         */
        iterator
        insert (iterator position, const std::shared_ptr<array> &value)
        {
            if (values_.empty () || values_[0]->is_array ())
            {
                return values_.insert (position, value);
    ...
    public:
         ...
         */
        using const_iterator
            = std::vector<std::shared_ptr<table>>::const_iterator;
         ...
         */
        void
        push_back (const std::shared_ptr<table> &val)
        {
         ...
         */
        iterator
        insert (iterator position, const std::shared_ptr<table> &value)
        {
    ...
    make_table_array (bool is_inline)
    {
    ...
    public:
         ...
         */
        bool
        contains_qualified (const std::string &key) const
        {
         ...
         */
        std::shared_ptr<base>
        get_qualified (const std::string &key) const
        {
         ...
         */
        std::shared_ptr<table>
        get_table (const std::string &key) const
        {
         ...
         */
        std::shared_ptr<table>
        get_table_qualified (const std::string &key) const
        {
         ...
         */
        std::shared_ptr<array>
        get_array_qualified (const std::string &key) const
        {
         ...
         */
        std::shared_ptr<table_array>
        get_table_array (const std::string &key) const
        {
            if (!contains (key)) return nullptr;
        ...
        template <class T>
        option<T>
        get_qualified_as (const std::string &key) const
        {
            try
            {
         ...
         */
        void
        document (const std::string &key, std::string doc)
        {
            doc_[key] = doc;
        }
         ...
         */
        void
        insert (const std::string &key, const std::shared_ptr<base> &value)
        {
            map_[key] = value;
        }
        ...
        template <class T>
        void
        insert (const std::string &key, T &&val,
                typename value_traits<T>::type * = 0)
        {
        ...
        bool
        has_doc (const std::string &key) const
        {
            return doc_.find (key) != doc_.end ();
        }

        std::string
        doc (const std::string &key) const
        {
    ...
    private:
        ...
        bool
        resolve_qualified (const std::string     &key,
    ...
    consumer<OnError>
    make_consumer (std::string::iterator &it, const std::string::iterator &end,
                   OnError &&on_error)
    {
    ...
    public:
         ...
         */
        parser (std::istream &stream, merge_type merge = merge_type::none,
                bool stringify_values = false)
            : input_ (stream), merge_ (merge),
              stringify_values_ (stringify_values)
        {
            // nothing
        }
    ...
    private:
        ...
        void
        parse_table (std::string::iterator       &it,
        ...
        void
        parse_single_table (std::string::iterator       &it,
        ...
        void
        parse_table_array (std::string::iterator       &it,
                           const std::string::iterator &end, table *&curr_table)
        {
            ++it;
            if (it == end || *it == ']')
            ...
            auto        key_part_handler = [&] (const std::string &part)
            {
                ...
                if (curr_table->contains (part))
                {
                    ...
                    if (it != end && *it == ']')
                    {
                        ...
                        if (v->is_inline ())
                        {
                            throw_parse_exception ("Static array "
                                                   + full_ta_name
                    ...
                    else
                    {
                        if (b->is_table ())
                            ...
                            curr_table
                                = std::static_pointer_cast<table_array> (b)
                                      ->get ()
                                      .back ()
                                      .get ();
                        else
        ...
        void
        parse_key_value (std::string::iterator &it, std::string::iterator &end,
                         table *curr_table)
        {
            auto key_end = [] (char c) { return c == '='; };
            ...
            auto key_part_handler = [&] (const std::string &part)
            {
                // two cases: this key part exists already, in which case it
                // must be a table, or it doesn't exist in which case we must
                // create an implicitly defined table
                if (curr_table->contains (part))
                {
        ...
        std::string
        parse_simple_key (std::string::iterator       &it,
                          const std::string::iterator &end)
        {
            consume_whitespace (it, end);
        ...
        parse_value (std::string::iterator &it, std::string::iterator &end)
        {
            ...
            if (stringify_values_)
            {
                ...
                switch (type)
                {
                    ...
                    case parse_type::LOCAL_TIME: parse_time (it, end); break;
                    ...
                    case parse_type::OFFSET_DATETIME:
                        ...
                        break;
                    case parse_type::INT:
                    case parse_type::FLOAT: parse_number (it, end); break;
                    case parse_type::BOOL: parse_bool (it, end); break;
                    default: is_value_type = false;
                }
        ...
        parse_type
        determine_value_type (const std::string::iterator &it,
                              const std::string::iterator &end)
        {
            if (it == end)
            {
        ...
        parse_type
        determine_number_type (const std::string::iterator &it,
                               const std::string::iterator &end)
        {
        ...
        std::string
        parse_escape_code (std::string::iterator       &it,
                           const std::string::iterator &end)
        {
            ++it;
        ...
        uint32_t
        parse_hex (std::string::iterator &it, const std::string::iterator &end,
                   uint32_t place)
        {
            uint32_t value = 0;
            while (place > 0)
        ...
        parse_array (std::string::iterator &it, std::string::iterator &end)
        {
            ...
            switch (type)
            {
                ...
                case parse_type::ARRAY:
                    return parse_object_array<array> (&parser::parse_array, '[',
                                                      it, end);
                case parse_type::INLINE_TABLE:
        ...
        parse_object_array (Function &&fun, char delim,
        ...
        std::shared_ptr<table>
        parse_inline_table (std::string::iterator &it,
                            std::string::iterator &end)
        {
            auto tbl = make_table ();
            do
        ...
        void
        skip_whitespace_and_comments (std::string::iterator &start,
                                      std::string::iterator &end)
        {
        ...
        void
        consume_whitespace (std::string::iterator       &it,
                            const std::string::iterator &end)
        {
            while (it != end && (*it == ' ' || *it == '\t'))
                ++it;
        ...
        void
        consume_backwards_whitespace (std::string::iterator       &back,
        ...
        void
        eol_or_comment (const std::string::iterator &it,
                        const std::string::iterator &end)
        {
            if (it != end && *it != '#')
        ...
        bool
        is_time (const std::string::iterator &it,
                 const std::string::iterator &end)
        {
            auto time_end = find_end_of_time (it, end);
        ...
        option<parse_type>
        date_type (const std::string::iterator &it,
                   const std::string::iterator &end)
        {
            auto date_end = find_end_of_date (it, end);
...
#endif
        if (!file.is_open ())
            throw parse_exception{ filename
                                   + " could not be opened for parsing" };
        parser p{ file, cpptoml::parser::merge_type::none };
        return p.parse ();
    }

    /** Parse base and override TOML from istream's and return the root table of
     * the merged result */
    inline std::shared_ptr<table>
    parse_base_and_override_files (const std::istream         &base,
                                   const std::istream         &override,
                                   cpptoml::parser::merge_type merge,
                                   bool stringify_values = false)
    {
        std::stringstream combined;
        combined << base.rdbuf ();
        // Validate base separately to get accurate line numbers
        parser            p_base{ combined, cpptoml::parser::merge_type::none,
                                  stringify_values };
        std::stringstream user;
        user << override.rdbuf ();
        // Validate user override file separately to get accurate line numbers
        parser p_user{ user, cpptoml::parser::merge_type::none,
                       stringify_values };
        p_user.parse ();
        // Combine
        combined << user.str ();
    ...
    /**
     * Parse a base and an override TOML and return the root table of the merged
     * result.
     */
    inline std::shared_ptr<table>
    parse_base_and_override_files (const std::string          &filename,
                                   ...
                                   bool stringify_values = false)
    {
        ...
        if (user_file.is_open ())
        {
            ...
            // Validate user override file separately to get accurate line
            // numbers
            parser p_user{ user, cpptoml::parser::merge_type::none,
                           stringify_values };
            p_user.parse ();
    ...
    template <> struct value_accept<>
    {
        template <class Visitor, class... Args>
        static void
        accept (const base &, Visitor &&, Args &&...)
        {
            // nothing
        }
    };
    ...
    template <class T, class... Ts> struct value_accept<T, Ts...>
    {
        template <class Visitor, class... Args>
        static void
        accept (const base &b, Visitor &&visitor, Args &&...args)
        {
            if (auto v = b.as<T> ())
            {
    ...
    /**
     * base implementation of accept() that calls visitor.visit() on the
     * concrete class.
     */
    template <class Visitor, class... Args>
    void
    base::accept (Visitor &&visitor, Args &&...args) const
    {
        if (is_value ())
        {
    ...
    public:
        ...
        void
        set_skip_empty_tables (bool enable)
        {
        ...
        template <class T>
        void
        visit (const value<T> &v, bool = false)
        {
            write (v);
        }
         ...
         */
        void
        visit (const table &t, bool in_array = false)
        {
         ...
         */
        void
        visit (const array &a, bool = false)
        {
            write ("[");
         ...
         */
        void
        visit (const table_array &t, bool = false)
        {
    ...
    protected:
         ...
         */
        void
        write (const value<std::string> &v)
        {
            write ("\"");
         ...
         */
        void
        write_table_header (bool in_array = false)
        {
            if (!path_.empty ())
            {
                indent ();
         ...
         */
        void
        write_table_item_header (const base &b)
        {
            if (!b.is_table () && !b.is_table_array ())
            {
                indent ();

                if (path_.back ().find_first_not_of (
                        "ABCDEFGHIJKLMNOPQRSTUVWXYZabcde"
                        "fghijklmnopqrstuvwxyz0123456789"
                        "_-")
                    == std::string::npos)
    ...
    inline std::ostream &
    operator<< (std::ostream &stream, const base &b)
    {
    ...
    template <class T>
    std::ostream &
    operator<< (std::ostream &stream, const value<T> &v)
    {
        toml_writer writer{ stream };
        v.accept (writer);
        return stream;
    ...
    inline std::ostream &
    operator<< (std::ostream &stream, const table &t)
    {
        toml_writer writer{ stream };
        t.accept (writer);
        return stream;
    ...
    inline std::ostream &
    operator<< (std::ostream &stream, const table_array &t)
    {
        toml_writer writer{ stream };
        t.accept (writer);
        return stream;
    ...
    inline std::ostream &
    operator<< (std::ostream &stream, const array &a)
    {
        toml_writer writer{ stream };
        a.accept (writer);
        return stream;
