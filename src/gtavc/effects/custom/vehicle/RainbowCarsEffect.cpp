...
static ThiscallEvent<AddressList<-1, H_CALL>, PRIORITY_BEFORE, ArgPickN<CVehicle*, 0>, void(CVehicle*)> setupRenderEvent;
static ThiscallEvent<AddressList<-1, H_CALL>, PRIORITY_AFTER, ArgPickN<CVehicle*, 0>, void(CVehicle*)> resetAfterRenderEvent;
...
void RainbowCarsEffect::Disable() {
	resetAfterRenderEvent -= ResetAfterRenderEvent;
	setupRenderEvent -= SetupRenderEvent;
...
void RainbowCarsEffect::HandleTick() {
...
void RainbowCarsEffect::SetupRenderEvent(CVehicle* vehicle) {
	ModifyCarPaint(vehicle);
}

void RainbowCarsEffect::ResetAfterRenderEvent(CVehicle* vehicle) {
...
void RainbowCarsEffect::ModifyCarPaint(CVehicle* vehicle) {
...
RpMaterial* RainbowCarsEffect::MaterialCallback(RpMaterial* material, void* data) {
	if (!data) {
		return material;
	}
	...
	resetMaterialColors.push_back(std::make_pair(&material->color, material->color));
...
RpAtomic* RainbowCarsEffect::AtomicCallback(RpAtomic* atomic, void* data) {
	if (atomic->geometry) {
